<script lang="ts">
	...
	async function login(event: SubmitEvent) {
		...
		try {
			...
			if (!response.ok) {
				errorMessage = resp.error || resp.message || `Login failed (${response.status}).`;
				console.log('Login failed', response.status, resp);
				return;
			}
	...
	<div class="relative z-10 w-full max-w-md" style="perspective: 1400px;">
		...
		>
			...
			<div class="flex items-center gap-1.5 border-b border-white/[0.09] px-5 py-3.5">
				...
				<span class="h-2.5 w-2.5 rounded-full bg-white/15"></span>
				<span class="h-2.5 w-2.5 rounded-full bg-white/15"></span>
				<span class="ml-3 font-mono text-[11px] text-white/30">stratum.app/login</span>
			</div>
			...
			<div class="px-8 py-9">
				...
				{#if errorMessage}
					<div
						class="mb-5 rounded-xl border border-[#ff3366]/30 bg-[#ff3366]/10 px-4 py-3 text-sm text-[#ff8fa8]"
					>
						{errorMessage}
					</div>
				{/if}
				...
				<p class="mt-6 text-center text-sm text-white/50">
					...
					<a href="/register" class="font-medium text-[#5db9f7] transition hover:text-white"
						>Sign up</a
					>
				</p>
			</div>
